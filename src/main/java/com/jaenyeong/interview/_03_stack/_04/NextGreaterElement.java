...
public class NextGreaterElement {
    ...
    public static void main(String[] args) {
        final NextGreaterElement nge = new NextGreaterElement();
    ...
    private int[] solution(int[] numbers) {
        ...
        final int nge[] = new int[numbers.length];

        for (int i = numbers.length - 1; i >= 0; i--) {
            while (!intStack.isEmpty() && intStack.peek() <= numbers[i]) {
                intStack.pop();
            }

            nge[i] = intStack.empty() ? NOT_FOUND : intStack.peek();
